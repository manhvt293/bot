...
namespace bot
{
    public abstract class BaseBot
    {
        ...
        private const string username = "manh.vuongtan";
        private const string password = "123456";
        private string token = "eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJtYW5oLnZ1b25ndGFuIiwiYXV0aCI6IlJPTEVfVVNFUiIsIkxBU1RfTE9HSU5fVElNRSI6MTY1MzQ2NDA2MTM1OCwiZXhwIjoxNjU1MjY0MDYxfQ.TV0C4ymWqGISWMxx7kbxpPRirUs0LC-3bU9wVIBof-9vGfBCwPitJjQrjAl9ycxjslniNg1kO1G0vWBgz1XCkQ";
        ...
        public void SendCastSkill(Hero heroCastSkill)
        {
            ...
            var myCES = botPlayer.GetHeroByID(HeroIdEnum.CERBERUS);
            var data = new SFSObject();

            data.PutUtfString("casterId", heroCastSkill.id.ToString());
            if (heroCastSkill.isHeroSelfSkill())
            {
                data.PutUtfString("targetId", myCES.id.ToString());
            }
            else
            {
                //giet tuong
                ...
                if (enemyPlayer.anyHeroFullMana() != null)
                {
                    data.PutUtfString("targetId", enemyPlayer.anyHeroFullMana().id.ToString());
                }
                else
                {
        ...
        {
            Pair<int> indexSwap = grid.recommendSwapGemFirstStrategy(botPlayer, enemyPlayer);
